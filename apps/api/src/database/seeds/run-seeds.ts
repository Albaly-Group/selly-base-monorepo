...
async function runSeeds() {
  ...
  const pgOptions: import('typeorm').DataSourceOptions = {
    type: 'postgres',
    host: String((dbConfig as any).host ?? process.env.DATABASE_HOST ?? 'localhost'),
    port: Number((dbConfig as any).port ?? process.env.DATABASE_PORT ?? 5432),
    username: String((dbConfig as any).username ?? process.env.DATABASE_USER ?? 'postgres'),
    password: String((dbConfig as any).password ?? process.env.DATABASE_PASSWORD ?? 'postgres'),
    database: String((dbConfig as any).database ?? process.env.DATABASE_NAME ?? 'selly_base'),
