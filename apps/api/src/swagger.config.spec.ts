describe('Swagger Configuration', () => {
  it('should handle serverless environment configuration correctly', () => {
    ...
    if (isServerless) {
      ...
      expect(swaggerConfig.customJs[1]).toContain(
  ...
  it('should use correct CDN URLs for all required assets', () => {
    ...
    expect(expectedCssUrl).toMatch(
      /^https:\/\/cdn\.jsdelivr\.net\/npm\/swagger-ui-dist@[\d.]+\/swagger-ui\.css$/,
    );
    expectedJsUrls.forEach((url) => {
      expect(url).toMatch(
        /^https:\/\/cdn\.jsdelivr\.net\/npm\/swagger-ui-dist@[\d.]+\/swagger-ui-.+\.js$/,
      );
    });
  });
});
